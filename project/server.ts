...
app.get('/api/schedules', async (req, res) => {
  try {
    const result = await oracle.select('SCHEDULE', '*');
    if (result.error) throw new Error(result.error);
    res.json({ data: result.data || [], error: null });
  } catch (error: any) {
...
app.post('/api/schedules', async (req, res) => {
  try {
    const result = await oracle.insert('SCHEDULE', req.body);
    if (result.error) throw new Error(result.error);
    res.status(201).json({ data: result.data, error: null });
  } catch (error: any) {
...
app.put('/api/schedules/:id', async (req, res) => {
  try {
    ...
    const idNumber = parseNumericId(id);
    if (idNumber === null) {
      return res.status(400).json({ data: null, error: 'Invalid schedule id' });
    }
    const result = await oracle.update('SCHEDULE', req.body, `id_schedule = ${idNumber}`);
    if (result.error) throw new Error(result.error);
    res.json({ data: result.data, error: null });
  } catch (error: any) {
...
app.delete('/api/schedules/:id', async (req, res) => {
  try {
    ...
    const result = await oracle.delete('SCHEDULE', `id_schedule = ${idNumber}`);
    if (result.error) throw new Error(result.error);
    res.json({ data: { success: true }, error: null });
  } catch (error: any) {
...
// ============ SCHEDULE_STOPS ROUTES ============

app.get('/api/schedule_stops', async (req, res) => {
  try {
    const result = await oracle.select('SCHEDULE_STOP', '*');
    if (result.error) throw new Error(result.error);
    res.json({ data: result.data || [], error: null });
  } catch (error: any) {
    console.error('GET /api/schedule_stops error:', error);
    res.status(500).json({ data: null, error: error.message });
  }
});

app.post('/api/schedule_stops', async (req, res) => {
  try {
    const result = await oracle.insert('SCHEDULE_STOP', req.body);
    if (result.error) throw new Error(result.error);
    res.status(201).json({ data: result.data, error: null });
  } catch (error: any) {
    console.error('POST /api/schedule_stops error:', error);
    res.status(500).json({ data: null, error: error.message });
  }
});

app.put('/api/schedule_stops/:scheduleId/:stationId', async (req, res) => {
  try {
    const { scheduleId, stationId } = req.params;
    const scheduleIdNumber = parseNumericId(scheduleId);
    const stationIdNumber = parseNumericId(stationId);

    if (scheduleIdNumber === null || stationIdNumber === null) {
      return res.status(400).json({ data: null, error: 'Invalid schedule or station id' });
    }

    const result = await oracle.update(
      'SCHEDULE_STOP',
      req.body,
      `id_schedule = ${scheduleIdNumber} AND id_station = ${stationIdNumber}`
    );
    if (result.error) throw new Error(result.error);
    res.json({ data: result.data, error: null });
  } catch (error: any) {
    console.error('PUT /api/schedule_stops error:', error);
    res.status(500).json({ data: null, error: error.message });
  }
});

app.delete('/api/schedule_stops/:scheduleId/:stationId', async (req, res) => {
  try {
    const { scheduleId, stationId } = req.params;
    const scheduleIdNumber = parseNumericId(scheduleId);
    const stationIdNumber = parseNumericId(stationId);
    ...
    const result = await oracle.delete(
      'SCHEDULE_STOP',
      `id_schedule = ${scheduleIdNumber} AND id_station = ${stationIdNumber}`
    );
    if (result.error) throw new Error(result.error);
    ...
    console.error('DELETE /api/schedule_stops error:', error);
    res.status(500).json({ data: null, error: error.message });
  }
});

// ============ LINE_STATIONS ROUTES ============

app.get('/api/line_stations', async (req, res) => {
  try {
    const result = await oracle.select('LINE_STATION', '*');
    if (result.error) throw new Error(result.error);
    res.json({ data: result.data || [], error: null });
  } catch (error: any) {
    console.error('GET /api/line_stations error:', error);
    res.status(500).json({ data: null, error: error.message });
  }
});

app.post('/api/line_stations', async (req, res) => {
  try {
    const result = await oracle.insert('LINE_STATION', req.body);
    if (result.error) throw new Error(result.error);
    res.status(201).json({ data: result.data, error: null });
  } catch (error: any) {
    console.error('POST /api/line_stations error:', error);
    res.status(500).json({ data: null, error: error.message });
  }
});

app.put('/api/line_stations/:lineId/:stationId', async (req, res) => {
  try {
    const { lineId, stationId } = req.params;
    const lineIdNumber = parseNumericId(lineId);
    const stationIdNumber = parseNumericId(stationId);

    if (lineIdNumber === null || stationIdNumber === null) {
      return res.status(400).json({ data: null, error: 'Invalid line or station id' });
    }

    const result = await oracle.update(
      'LINE_STATION',
      req.body,
      `id_line = ${lineIdNumber} AND id_station = ${stationIdNumber}`
    );
    if (result.error) throw new Error(result.error);
    res.json({ data: result.data, error: null });
  } catch (error: any) {
    console.error('PUT /api/line_stations error:', error);
    res.status(500).json({ data: null, error: error.message });
  }
});

app.delete('/api/line_stations/:lineId/:stationId', async (req, res) => {
  try {
    const { lineId, stationId } = req.params;
    const lineIdNumber = parseNumericId(lineId);
    const stationIdNumber = parseNumericId(stationId);
    ...
    console.error('DELETE /api/line_stations error:', error);
    res.status(500).json({ data: null, error: error.message });
  }
});
...
app.listen(PORT, () => {
  ...
  console.log(`   - /api/schedule_stops`);
  console.log(`   - /api/line_stations`);
});