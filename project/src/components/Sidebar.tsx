...
const menuItems = [
  ...
  { icon: Clock, label: 'Schedules', path: '/schedules', entity: 'schedules' },
  { icon: Clock, label: 'Schedule Stops', path: '/schedule-stops', entity: 'scheduleStops' },
  { icon: MapPin, label: 'Line Stations', path: '/line-stations', entity: 'lineStations' },
];
    ...
    <>
      <div className="px-6 py-5 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            {!isCollapsed && (
              <>
                <div className="flex items-center justify-center flex-shrink-0 w-10 h-10 bg-blue-400 rounded-lg">
      ...
      <div className="px-3 py-4 space-y-1 border-t border-gray-200 dark:border-gray-700">
        {user && (
          <button
            onClick={logout}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 ${isCollapsed && !isMobile ? 'justify-center' : ''
              }`}
            title="Se déconnecter"
          >
            <LogOut className="flex-shrink-0 w-5 h-5" />
            {(!isCollapsed || isMobile) && <span>Se déconnecter</span>}
          </button>
        )}
        <button
          onClick={toggleTheme}
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed z-40 p-2 bg-white rounded-lg shadow-lg dark:bg-gray-800 lg:hidden top-4 left-4"
      >
