...
export const ROLE_DEFINITIONS: Record<RoleKey, RoleDefinition> = {
    role_administrateur: {
        ...
        routes: ['/dashboard', '/cities', '/stations', '/bus-lines', '/buses', '/drivers', '/trips', '/tickets', '/subscriptions', '/maintenance', '/incidents', '/schedules', '/schedule-stops', '/line-stations'],
        permissions: {
            dashboard: ['view'],
            ...
            scheduleStops: ['view', 'create', 'update', 'delete'],
            lineStations: ['view', 'create', 'update', 'delete'],
        },
    },
    role_gestionnaire: {
        label: 'Gestionnaire',
        routes: ['/cities', '/stations', '/bus-lines', '/buses', '/drivers', '/trips', '/tickets', '/subscriptions', '/schedules', '/schedule-stops', '/line-stations'],
        permissions: {
            ...
            scheduleStops: ['view', 'create', 'update'],
            lineStations: ['view', 'create', 'update'],
        },
    },
    role_chauffeur_consultation: {
        label: 'Chauffeur',
        routes: ['/trips', '/incidents', '/schedules', '/schedule-stops'],
        permissions: {
            trips: ['view'],
    ...
    role_maintenance: {
        label: 'Maintenance',
        routes: ['/buses', '/maintenance', '/incidents'],
        permissions: {
            buses: ['view'],
            maintenance: ['view', 'create', 'update', 'delete'],
    ...
    role_commercial: {
        label: 'Commercial',
        routes: ['/tickets', '/subscriptions', '/trips', '/schedules'],
        permissions: {
            tickets: ['view', 'create', 'update'],
            subscriptions: ['view', 'create', 'update'],
            trips: ['view'],
...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    const login = async (username: string, password: string): Promise<{ nextRoute: string }> => {
        try {
            ...
            const roleRoutes = ROLE_DEFINITIONS[userRole]?.routes || [];
            const nextRoute = roleRoutes.length > 0 ? roleRoutes[0] : '/';
