...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const fetchSchedules = async () => {
    try {
      const response = await api.from('schedules').select('*');
      if (response.data && Array.isArray(response.data)) {
  ...
  const fetchScheduleStops = async () => {
    try {
      const response = await api.from('schedule_stops').select('*');
      if (response.data && Array.isArray(response.data)) {
        setScheduleStops(response.data);
      }
    } catch (error) {
  ...
  const fetchLineStations = async () => {
    try {
      const response = await api.from('line_stations').select('*');
      if (response.data && Array.isArray(response.data)) {
        const sorted = [...response.data].sort((a, b) =>
          (a.stop_order || 0) - (b.stop_order || 0)
        );
        setLineStations(sorted);
      }
    } catch (error) {
  ...
  const fetchData = async (entity: string) => {
    const fetchMap: Record<string, () => Promise<void>> = {
      ...
      scheduleStops: fetchScheduleStops,
      lineStations: fetchLineStations,
